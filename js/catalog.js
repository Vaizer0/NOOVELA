  ...
  try{
    ...
    if(addBtn){
      ...
      addBtn.onclick=async()=>{
        if(inLib){if(book.id)await DB.deleteBook(book.id);addBtn.textContent='Add to Library';showToast('Removed from library','info');}
        else{const saved=await DB.saveBook(Object.assign({},book,{addedAt:Date.now(),updatedAt:Date.now(),sourceId:sourceId||book.sourceId}));_biBook=saved;if(chapters.length)await DB.saveChapters(saved.id,chapters);addBtn.textContent='Remove from Library';showToast('Added to library','success');}
...
function _renderChapters(book,chapters,sourceId){
  ...
  container.innerHTML='';
  if(!chapters||!chapters.length){const p=document.createElement('p');p.style.cssText='color:var(--text2);padding:12px;';p.textContent='No chapters found.';container.appendChild(p);return;}
  chapters.forEach((ch,idx)=>{
    ...
    div.addEventListener('click',async()=>{
      ...
      if(bookRef&&!await DB.getBook(bookRef.id||'').catch(()=>null)){bookRef=await DB.saveBook(Object.assign({},bookRef,{addedAt:Date.now(),updatedAt:Date.now(),sourceId:sourceId||bookRef.sourceId}));await DB.saveChapters(bookRef.id,chapters);}
...
function openFinder(){
  ...
  if(!plugins.length){const p=document.createElement('div');p.className='empty-state';const t=document.createElement('p');t.textContent='No extensions installed. Go to Extensions to add one.';p.appendChild(t);sourcesEl.appendChild(p);return;}
  plugins.forEach(ext=>{
    const card=document.createElement('div');card.className='source-card';
    const name=document.createElement('div');name.className='source-name';name.textContent=ext.name||ext.id||'Unknown';
    const meta=document.createElement('div');meta.className='source-meta';meta.textContent=(ext.lang||'')+(ext.version?' v'+ext.version:'');
    if(ext.iconUrl){const icon=document.createElement('img');icon.className='source-icon';icon.src=ext.iconUrl;icon.width=24;icon.height=24;icon.onerror=function(){this.style.display='none';};card.appendChild(icon);}
    card.appendChild(name);card.appendChild(meta);
    card.addEventListener('click',()=>{_finderPlugin=ext;if(title)title.textContent=ext.name||ext.id;sourcesEl.style.display='none';_loadFinderPopular();});
...
async function _loadFinderPopular(){
  ...
  try{const results=await Plugins.getPopular(_finderPlugin.id,1);_renderFinderResults(Array.isArray(results)?results:[]);}
  catch(e){if(resultsEl)resultsEl.innerHTML='<div class="error-state">Failed to load: '+e.message+'</div>';}
}
async function finderSearch(query){
  ...
  try{const results=await Plugins.searchNovels(_finderPlugin.id,query,1);_renderFinderResults(Array.isArray(results)?results:[]);}
...
function _renderFinderResults(results){
  ...
  container.innerHTML='';
  if(!results.length){const e=document.createElement('div');e.className='empty-state';const p=document.createElement('p');p.textContent='No results found.';e.appendChild(p);container.appendChild(e);return;}
  results.forEach(book=>{
    ...
    if(imgSrc){const img=document.createElement('img');img.className='search-result-cover';img.dataset.src=imgSrc;img.alt=book.title||book.name||'';img.onerror=function(){imgFallback(this);};div.appendChild(img);}
    const info=document.createElement('div');info.className='search-result-info';
    const t=document.createElement('div');t.className='search-result-title';t.textContent=truncate(book.title||book.name||'',60);
    ...
    div.addEventListener('click',()=>{const normalized={title:book.title||book.name||'',name:book.name||book.title||'',coverUrl:book.coverUrl||book.cover||book.thumbnail||'',url:book.url||book.path||'',path:book.path||'',author:book.author||'',genres:book.genres||book.genre||'',description:book.description||book.summary||'',sourceId:_finderPlugin?_finderPlugin.id:null,type:'novel'};openBookInfo(null,normalized,_finderPlugin?_finderPlugin.id:null);});
    container.appendChild(div);
  });
  lazyLoad(container);
}
function longPressInit(el,callback,delay){delay=delay||600;let timer=null;el.addEventListener('touchstart',e=>{timer=setTimeout(()=>{callback(e);timer=null;},delay);},{passive:true});el.addEventListener('touchend',()=>{if(timer){clearTimeout(timer);timer=null;}});el.addEventListener('touchmove',()=>{if(timer){clearTimeout(timer);timer=null;}},{passive:true});el.addEventListener('contextmenu',e=>{e.preventDefault();callback(e);});}
