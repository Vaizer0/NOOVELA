...
const DB={
  ...
  async saveBook(b){b.updatedAt=Date.now();if(!b.id)b.id='book_'+Date.now()+'_'+Math.random().toString(36).slice(2);await txPut('books',b);return b;},
  async deleteBook(id){const chaps=await this.getChapters(id);for(const c of chaps){await txDel('content',c.id);await txDel('downloads',c.id);await txDel('chapters',c.id);}await txDel('progress',id);await txDel('books',id);},
  getChapters:bookId=>txAll('chapters','bookId',bookId),
  getChapter:id=>txGet('chapters',id),
  async saveChapter(c){if(!c.id)c.id='chap_'+Date.now()+'_'+Math.random().toString(36).slice(2);return txPut('chapters',c);},
  async saveChapters(bookId,chapters){
    const existing=await this.getChapters(bookId);
    const map=new Map(existing.map(c=>[c.url,c]));
    for(const ch of chapters){const ex=map.get(ch.url);if(ex)await txPut('chapters',{...ex,...ch,id:ex.id,read:ex.read,bookId});else await this.saveChapter({...ch,bookId,read:false,downloaded:false});}
  },
  ...
  getChapterContent:cid=>txGet('content',cid).then(r=>r?r.content:null),
  saveChapterContent:(cid,content)=>txPut('content',{chapterId:cid,content,savedAt:Date.now()}),
  ...
  async getHistory(limit){const all=await txAll('history');const sorted=all.sort((a,b)=>b.readAt-a.readAt);return limit?sorted.slice(0,limit):sorted;},
  ...
  async exportAll(){
    const [books,chapters,content,history,progress,settings,categories,extensions,regexRules]=await Promise.all([txAll('books'),txAll('chapters'),txAll('content'),txAll('history'),txAll('progress'),txAll('settings'),txAll('categories'),txAll('extensions'),txAll('regexRules')]);
