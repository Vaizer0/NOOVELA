...
function _cheerioLoad(html){
  ...
  function _wrap(nodes){
    ...
    const $obj={
      ...
      attr(n,v){if(v!==undefined){arr.forEach(el=>el.setAttribute&&el.setAttribute(n,String(v)));return $obj;}return arr.length&&arr[0].getAttribute?arr[0].getAttribute(n):undefined;},
...
function _dayjs(val){
  ...
  const obj={
    format(fmt){return String(fmt||'').replace(/YYYY/,d.getFullYear()).replace(/YY/,String(d.getFullYear()).slice(-2)).replace(/MM/,pad(d.getMonth()+1)).replace(/DD/,pad(d.getDate())).replace(/HH/,pad(d.getHours())).replace(/hh/,pad(d.getHours()%12||12)).replace(/mm/,pad(d.getMinutes())).replace(/ss/,pad(d.getSeconds())).replace(/A/,d.getHours()<12?'AM':'PM').replace(/a/,d.getHours()<12?'am':'pm');},
    ...
    diff(other,unit){const od=other&&other.toDate?other.toDate():(other?new Date(other):new Date());const ms=d-od;if(unit==='day'||unit==='days')return Math.floor(ms/86400000);if(unit==='hour'||unit==='hours')return Math.floor(ms/3600000);if(unit==='minute'||unit==='minutes')return Math.floor(ms/60000);return ms;},
    ...
    startOf(unit){const nd=new Date(d);if(unit==='day'){nd.setHours(0,0,0,0);}else if(unit==='month'){nd.setDate(1);nd.setHours(0,0,0,0);}return _dayjs(nd);},
    get(unit){if(unit==='year')return d.getFullYear();if(unit==='month')return d.getMonth();if(unit==='date'||unit==='day')return d.getDate();return 0;},
  };
  return obj;
}
function _unitMs(unit){if(unit==='year'||unit==='years')return 365*86400000;if(unit==='month'||unit==='months')return 30*86400000;if(unit==='week'||unit==='weeks')return 7*86400000;if(unit==='day'||unit==='days')return 86400000;if(unit==='hour'||unit==='hours')return 3600000;if(unit==='minute'||unit==='minutes')return 60000;return 1000;}
...
class _PluginStorage{
  ...
  set(key,value,expires){const exp=expires instanceof Date?expires.getTime():(typeof expires==='number'?Date.now()+expires:undefined);try{localStorage.setItem(this._k(key),JSON.stringify({value,created:Date.now(),expires:exp}));}catch(e){}}
  get(key){try{const raw=localStorage.getItem(this._k(key));if(!raw)return undefined;const item=JSON.parse(raw);if(item.expires&&Date.now()>item.expires){this.delete(key);return undefined;}return item.value;}catch(e){return undefined;}}
...
class _LocalStorage{constructor(id){this._id=id;}get(){try{const v=localStorage.getItem('__pls_'+this._id);return v?JSON.parse(v):undefined;}catch(e){return undefined;}}}
class _SessionStorage{constructor(id){this._id=id;}get(){try{const v=sessionStorage.getItem('__pss_'+this._id);return v?JSON.parse(v):undefined;}catch(e){return undefined;}}}
...
function _makeRequire(pluginId){
  const pkgs={
    ...
    '@libs/novelStatus':{NovelStatus:{Unknown:'Unknown',Ongoing:'Ongoing',Completed:'Completed',Licensed:'Licensed',PublishingFinished:'Publishing Finished',Cancelled:'Cancelled',OnHiatus:'On Hiatus'}},
    ...
    '@libs/utils':{utf8ToBytes:(s)=>new TextEncoder().encode(s),bytesToUtf8:(b)=>new TextDecoder().decode(b)},
  };
  return (pkg)=>{
    if(pkg==='@libs/storage')return{storage:new _PluginStorage(pluginId),localStorage:new _LocalStorage(pluginId),sessionStorage:new _SessionStorage(pluginId)};
    const m=pkgs[pkg];if(m!==undefined)return m;
...
function _absUrl(base,path){if(!path)return '';if(/^https?:\/\//.test(path))return path;try{return new URL(path,base).href;}catch(e){return(base||'').replace(/\/$/,'')+'/'+path.replace(/^\/+/,'');}}

function _normalizeNovels(novels,ext){const site=ext&&ext.plugin&&ext.plugin.site?ext.plugin.site:(ext&&ext.site?ext.site:'');const sourceId=ext&&ext.id?ext.id:'';return (novels||[]).map(n=>({title:n.name||n.title||'',coverUrl:n.cover||n.coverUrl||n.thumbnail||'',url:n.path?_absUrl(site,n.path):(n.url||''),path:n.path||'',author:n.author||'',genres:Array.isArray(n.genres)?n.genres.join(', '):(n.genres||n.genre||''),description:n.summary||n.description||'',status:n.status||'',sourceId,type:'novel'}));}

function _normalizeChapters(chapters,ext){const site=ext&&ext.plugin&&ext.plugin.site?ext.plugin.site:(ext&&ext.site?ext.site:'');return (chapters||[]).map((ch,idx)=>({title:ch.name||ch.title||('Chapter '+(idx+1)),url:ch.path?_absUrl(site,ch.path):(ch.url||''),path:ch.path||'',chapterNumber:ch.chapterNumber||(idx+1),releaseTime:ch.releaseTime||ch.updatedAt||'',read:false}));}

// Exact lnreader pluginManager.ts compile pattern
function _compile(ext){
  ...
  try{
    const plugin=Function('require','module',`const exports = module.exports = {};\n${ext.code};\nreturn exports.default`)(req,{});
    if(plugin&&typeof plugin==='object'&&(typeof plugin.popularNovels==='function'||typeof plugin.searchNovels==='function'||typeof plugin.parseNovel==='function'||typeof plugin.parseChapter==='function')){
      ext.plugin=plugin;ext.type='lnreader';
    ...
    }else{ext.type='legacy';ext.plugin=null;ext.mod=plugin||{};}
  }catch(e){
    console.error('[Plugin compile error]',pluginId,e.message,e.stack);
    ext.type='error';ext.plugin=null;ext.mod={};
  }
...
const PluginManager={
  async loadFromDB(){try{const exts=await DB.getAllExtensions().catch(()=>[]);for(const e of(exts||[]))_compile(e);}catch(e){console.error('PluginManager.loadFromDB:',e);}},
  ...
  async installFromUrl(url,meta){
    ...
    try{
      ...
      const ext=Object.assign({name:id,lang:'',version:'',iconUrl:'',site:''},meta||{},{id,code,sourceUrl:url});
      _compile(ext);await DB.saveExtension(ext);
  ...
  async getPopular(id,page){
    const ext=_store[id];if(!ext)throw new Error('Plugin not found: '+id);
    if(ext.type==='lnreader'&&ext.plugin&&typeof ext.plugin.popularNovels==='function'){const novels=await ext.plugin.popularNovels(page||1,{showLatestNovels:false,filters:{}});return _normalizeNovels(Array.isArray(novels)?novels:[],ext);}
    if(ext.mod&&typeof ext.mod.fetchPopular==='function'){const res=await ext.mod.fetchPopular(page||1);return Array.isArray(res)?res:[];}
    if(ext.mod&&typeof ext.mod.fetchLatest==='function'){const res=await ext.mod.fetchLatest(page||1);return Array.isArray(res)?res:[];}
    throw new Error('No popularNovels method in plugin: '+id);
  },
  async searchNovels(id,term,page){
    const ext=_store[id];if(!ext)throw new Error('Plugin not found: '+id);
    if(ext.type==='lnreader'&&ext.plugin&&typeof ext.plugin.searchNovels==='function'){const novels=await ext.plugin.searchNovels(term,page||1);return _normalizeNovels(Array.isArray(novels)?novels:[],ext);}
    if(ext.mod&&typeof ext.mod.search==='function'){const res=await ext.mod.search(term,page||1);return Array.isArray(res)?res:[];}
  ...
  async getNovelDetails(id,url){
    ...
    if(ext.type==='lnreader'&&ext.plugin&&typeof ext.plugin.parseNovel==='function'){const res=await ext.plugin.parseNovel(path);const chapters=_normalizeChapters(res.chapters||[],ext);return{title:res.name||res.title||'',coverUrl:res.cover||res.coverUrl||'',author:res.author||'',artist:res.artist||'',status:typeof res.status==='string'?res.status:'',genres:Array.isArray(res.genres)?res.genres.join(', '):(res.genres||''),description:res.summary||res.description||'',chapters,sourceId:id,type:'novel',url};}
    if(ext.mod&&typeof ext.mod.getBookInfo==='function')return await ext.mod.getBookInfo(url);
  ...
  async getChapterContent(id,chapterUrl){
    ...
    if(ext.type==='lnreader'&&ext.plugin&&typeof ext.plugin.parseChapter==='function'){const res=await ext.plugin.parseChapter(path);if(typeof res==='string')return res;return res&&(res.chapterText||res.text||res.html||res.content)||'';}
    if(ext.mod&&typeof ext.mod.fetchChapter==='function')return await ext.mod.fetchChapter(chapterUrl);
  ...
  async fetchLnReaderIndex(customUrl){
    ...
    try{const json=await Scraper.fetchText(url);const arr=JSON.parse(json);return arr.map(item=>({id:String(item.id||''),name:String(item.name||''),lang:String(item.lang||''),version:String(item.version||''),sourceUrl:String(item.url||''),iconUrl:String(item.iconUrl||''),site:String(item.site||'')}));}
    catch(e){console.error('fetchLnReaderIndex:',e);return [];}
  },
  async fetchIndex(customUrl){
    const url=customUrl||EXT_INDEX_URL;
    try{const yaml=await Scraper.fetchText(url);return this._parseYaml(yaml);}catch(e){console.error('fetchIndex:',e);return [];}
  },
  _parseYaml(yaml){
