...
async function _rdrFetch(url,book){
  if(book&&book.sourceId){
    try{const content=await Plugins.getChapterContent(book.sourceId,url);if(content&&content.length>20)return content;}
    catch(e){console.warn('Plugin getChapterContent failed:',e.message);}
  }
  try{const dom=await Scraper.fetchDom(url);return dom?Scraper.extractText(dom):'';}
...
function _applyReaderSettings(){const rs=document.getElementById('screen-reader');if(!rs)return;rs.style.setProperty('--rd-size',(Settings.get('fontSize')||16)+'px');rs.style.setProperty('--rd-font',Settings.get('fontFamily')||'Georgia,serif');rs.style.setProperty('--rd-lh',String(Settings.get('lineHeight')||1.8));rs.style.setProperty('--rd-ps',(Settings.get('paragraphSpacing')||12)+'px');rs.style.setProperty('--rd-ta',Settings.get('textAlign')||'left');const bg=Settings.get('readerBg');if(bg&&bg!=='default')rs.style.setProperty('--reader-bg',bg);}
function _rdrUpdateNav(){const prev=document.getElementById('reader-prev-btn'),next=document.getElementById('reader-next-btn');if(prev)prev.disabled=(_rdrIdx<=0);if(next)next.disabled=(_rdrIdx>=_rdrChapters.length-1);}
function _rdrUpdateTitle(){const el=document.getElementById('reader-chapter-title'),ch=_rdrChapters[_rdrIdx];if(el)el.textContent=ch?(ch.title||ch.name||''):(_rdrBook?(_rdrBook.title||_rdrBook.name||''):'');}
function readerNavChapter(dir){const idx=_rdrIdx+dir;if(idx<0||idx>=_rdrChapters.length){showToast('No more chapters','info');return;}openReader(_rdrBook,(_rdrChapters[idx].url||_rdrChapters[idx].path),_rdrChapters);}
async function readerTranslate(){const container=document.getElementById('reader-content');if(!container)return;const paras=Array.from(container.querySelectorAll('.reader-para'));if(!paras.length)return;showLoading('Translating...');try{const texts=paras.map(p=>p.textContent||'');const translated=await Translation.translateBatch(texts);const mode=Settings.get('translateDisplayMode')||'replace';if(mode==='replace'){paras.forEach((p,i)=>{p.textContent=translated[i]||p.textContent;});}else{paras.forEach((p,i)=>{const tr=document.createElement('div');tr.className='reader-para-trans';tr.textContent=translated[i]||'';p.insertAdjacentElement('afterend',tr);});}TTS.prepareContainer(container);}catch(e){showToast('Translation error: '+e.message,'error');}finally{hideLoading();}
}
